...
for i in range(max_iter+1):
  ...
  pairs_distance = pairs.map(lambda x: (x[0][0],(x[0][1], x[1][0],np.sum((x[0][1]-x[1][1])**2))))
  ...
  # count how many times each centroid is used
  centroid_count = all_points_centroids.map(lambda x: (x[0],1)).reduceByKey(lambda x,y: x+y)

  # sum all of the point coordinates associated with each centroid 
  coordinate_sum = all_points_centroids.reduceByKey(lambda x,y: x+y)

  # average over the points to create new centroids
  centroids = coordinate_sum.join(centroid_count).map(lambda x: (x[0],x[1][0]/x[1][1]))
  ...
  centroid_count.unpersist()
  coordinate_sum.unpersist()
...
plt.savefig('Cost_c1_Euclidean.png')
#plt.show()
